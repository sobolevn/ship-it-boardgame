...
И прямо как в настоящей жизни, нужно будет создавать ситуативные команды, а затем их предавать, мониторить свою инфраструктуру и не деплоить в пятницу, совершать нечестные обмены или просто воровать. С нешуточными последствиями!
...
- С дополнительными картами (Кеш, OpenSource, Цепочка поставок, Влить) или без
...
Есть 4 карты, которые можно добавлять или убирать из игры: "Кеш", "OpenSource", "Цепочка поставок" и "Влить". Они привносят немного хаоса и веселья! Можете играть с ними или без них.
...
Карта "Неа" может полностью отменить действие любой карты из категорий "Компонент", "Добор" и "Мета".
Карта "Неа" разыгрывается только в ход другого игрока.

Карты категории "Компонент" отправляются обратно в руку, другие – отправляются в сброс.

Пример: Алиса при помощи карты "RFC" просит Бориса отдать ей карту, Борис разыгрывает карту "Неа", карты "RFC" и "Неа" идут в сброс.

Компонент, отмененный картой "Неа", можно деплоить еще раз в следующий ход.
